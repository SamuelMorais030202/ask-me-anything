...
func (q *Queries) ReactToMessage(ctx context.Context, id uuid.UUID) (int64, error) {
	...
	var creation_count int64
	err := row.Scan(&creation_count)
	return creation_count, err
}
...
func (q *Queries) getRom(ctx context.Context, id uuid.UUID) (Room, error) {
	row := q.db.QueryRow(ctx, getRom, id)
	var i Room
	err := row.Scan(&i.ID, &i.Theme)
	return i, err
}
